...
describe('CollectionUniqueNFTs', () => {
  ...
  before(async () => {
    ...
    marketplaceContract = await aeSdk.getContractInstance({ source: marketplaceContractSource, fileSystem: utils.getFilesystem(MARKETPLACE_CONTRACT_SOURCE) });
  ...
  describe('NFT marketplace interactions', async () => {
    it('announce, cancel & execute a sale', async () => {
      const marketplaceContractId = marketplaceContract.deployInfo.address;
      const marketplaceContractAddress = marketplaceContractId.replace("ct_", "ak_");

      // approve marketplace contract
      let approveTx = await contract.methods.approve(marketplaceContractAddress, 1, true, { onAccount: accounts[0] });
      ...
      let announceSaleTx = await marketplaceContract.methods.announce_sale(contract.deployInfo.address, 1, 10, { onAccount: accounts[0] });
      assert.equal(announceSaleTx.decodedEvents[0].name, 'AnnounceSale');
      assert.equal(announceSaleTx.decodedEvents[0].args[0], await accounts[0].address());
      assert.equal(announceSaleTx.decodedEvents[0].args[1], 1);

      // expected failed cancellation
      await expect(
        marketplaceContract.methods.cancel_sale(1, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "SALE_CAN_ONLY_BE_CANCELLED_BY_SELLER"`);
      const cancelSaleTx = await marketplaceContract.methods.cancel_sale(1, { onAccount: accounts[0] });
      ...
      approveTx = await contract.methods.approve(marketplaceContractAddress, 1, true, { onAccount: accounts[0] });
      announceSaleTx = await marketplaceContract.methods.announce_sale(contract.deployInfo.address, 1, 10, { onAccount: accounts[0] });
      assert.equal(announceSaleTx.decodedEvents[0].name, 'AnnounceSale');
      assert.equal(announceSaleTx.decodedEvents[0].args[0], await accounts[0].address());
      ...
      await expect(
        marketplaceContract.methods.execute_sale(1, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "SALE_NOT_IN_STATUS_OPEN"`);
      await expect(
        marketplaceContract.methods.execute_sale(2, { onAccount: accounts[1], amount: 9 }))
        .to.be.rejectedWith(`Invocation failed: "PROVIDED_AMOUNT_NOT_MATCHES_PRICE"`);
      await expect(
        marketplaceContract.methods.execute_sale(2, { onAccount: accounts[1], amount: 11 }))
        .to.be.rejectedWith(`Invocation failed: "PROVIDED_AMOUNT_NOT_MATCHES_PRICE"`);

      // execute the trade marketplace contract
      const executeSaleTx = await marketplaceContract.methods.execute_sale(2, { onAccount: accounts[1], amount: 10 });
      assert.equal(executeSaleTx.decodedEvents[0].name, 'ExecuteSale');
      assert.equal(executeSaleTx.decodedEvents[0].args[0], await accounts[0].address());
      assert.equal(executeSaleTx.decodedEvents[0].args[1], await accounts[1].address());
      ...
      // check new owner
      const ownerDryRun = await contract.methods.owner(1);
      assert.equal(ownerDryRun.decodedResult, await accounts[1].address());
    });
  });
});
